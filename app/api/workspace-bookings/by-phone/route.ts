...
/**
 * GET /api/workspace-bookings/by-phone?phone=+91XXXXXXXXXX
 * Retrieves the most recent workspace booking for a given phone number
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      console.error('❌ Supabase error fetching workspace booking by phone:', error);
      return NextResponse.json(
    ...
    console.log(`✅ Fetched workspace booking for phone: ${phone}`);
    return NextResponse.json({ data }, { status: 200 });
  } catch (error: any) {
    console.error('❌ Error fetching workspace booking by phone:', error);
    return NextResponse.json(
